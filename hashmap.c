...
void enlarge(HashMap * map) {
    ...
    Pair ** aux;
    aux=map->buckets;
    long capo=map->capacity;
    map->buckets=(Pair**) calloc(capo*2,sizeof(HashMap));
    map->capacity=capo*2;
    for(long idx=0;idx<capo; idx++){
        if(aux[idx]!=NULL && aux[idx]->key!=NULL){
            insertMap(map, aux[idx]->key, aux[idx]->value);
        }
    }
}
